...
// Update provides a mock function with given fields: token, updateData, image
func (_m *UserService) Update(token interface{}, updateData user.Core, image *multipart.FileHeader) (user.Core, error) {
	ret := _m.Called(token, updateData, image)

	var r0 user.Core
	if rf, ok := ret.Get(0).(func(interface{}, user.Core, *multipart.FileHeader) user.Core); ok {
		r0 = rf(token, updateData, image)
	} else {
		r0 = ret.Get(0).(user.Core)
	}
